...
model_fit <- bind_rows(list(gs_d13c = gs_d13c, 
                            gs_d15n = gs_d15n,
                            gs_d15n_outlier = gs_d15n_outlier), 
                       .id = "id") %>% 
  mutate(
    metric = "temp"
  ) %>% 
...
model_effects <- bind_rows(list(me_d13c = me_d13c, 
                                me_d15n = me_d15n,
                                me_d15n_outlier = me_d15n_outlier), 
                           .id = "id") %>% 
  mutate(
    metric = "temp"
  ) %>% 
...
#   n_15 = seq(8.75, 12, length.out = 100),
#   mean_temp = seq(4.25, 7.75, length.out = 100)
# 
# )
# # 
...
# 
# 
# ggplot(data = preds, aes(x = mean_temp, y = fit)) + 
#   geom_(aes(fill = fit), linewidth = 1) + 
